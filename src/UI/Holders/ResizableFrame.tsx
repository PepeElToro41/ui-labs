...
function ResizableFrame(setprops: ResizableFrameProps) {
	...
	const [mouseInput, setMouseInput] = useState<InputObject | undefined>(
		undefined,
	);
	const [resizeAdd, setResizeAdd] = useBinding(new Vector2(0, 0));
	const [absSize, setAbsSize] = useState<Vector2 | undefined>(undefined);

	const isWidthResizable =
		props.HandleAnchor === "Left" || props.HandleAnchor === "Right";
	const isHeightResizable =
		props.HandleAnchor === "Top" || props.HandleAnchor === "Bottom";

	const resizeMultiply = new Vector2(
		props.HandleAnchor === "Left" ? -1 : 1,
		props.HandleAnchor === "Top" ? -1 : 1,
	).mul(new Vector2(isWidthResizable ? 1 : 0, isHeightResizable ? 1 : 0));
	...
	useBindingListener(resizeAdd, (adder) => {
		...
		if (!collapsed) {
			if (
				adder.X < props.MaxBeforeCollapse ||
				adder.Y < props.MaxBeforeCollapse
			) {
				setCollapsed(true);
			}
		} else {
		...
		if (!isDragging) {
			if (collapsed) {
				...
				setResizeAdd(
					absSize.mul(
						new Vector2(
							-math.abs(resizeMultiply.X),
							-math.abs(resizeMultiply.Y),
						),
					),
				);
			}
	...
	const InformResized = (added?: Vector2) => {
		...
		if (props.OnResized) {
			props.OnResized(
				collapsed,
				math.clamp(
					setAdded.X + setAdded.Y,
					props.ResizeRange.Min,
					props.ResizeRange.Max,
				),
			);
		}
	};
	...
	const dragHandleX =
		props.HandleAnchor === "Left"
			? 0
			: props.HandleAnchor === "Right"
				? 1
				: 0.5;
	const dragHandleY =
		props.HandleAnchor === "Top"
			? 0
			: props.HandleAnchor === "Bottom"
				? 1
				: 0.5;
	...
	return (
		...
		>
			...
			<frame
				...
				AnchorPoint={
					new Vector2(
						isDragging ? 0.5 : 1 - dragHandleX,
						isDragging ? 0.5 : 1 - dragHandleY,
					)
				}
				BorderSizePixel={0}
				...
				Size={
					isWidthResizable
						? new UDim2(0, isDragging ? 2 : 1, 1, 0)
						: new UDim2(1, 0, 0, isDragging ? 2 : 1)
				}
				ZIndex={2}
			></frame>
			<Detector
				...
				Size={
					isWidthResizable
						? new UDim2(0, props.HandleSize, 1, 0)
						: new UDim2(1, 0, 0, props.HandleSize)
				}
				ZIndex={3}
				Event={{
					MouseLeave: () => {
